...
MMU::MMU(std::string filename, HeaderInfo *hi) {
	hi_ = hi;
	loadROM(filename);
...
void MMU::writeByte(WORD address, BYTE val) {
	...
	if (address < 0x2000) {
		...
		case 0x07: // TAC register, timer control 0xFF07
			...
			// turn timer on or off based on bit 2
			timer_running_ = (val >> 2) & 0x01;
			// select the clock based on bits 0-1
...
void MMU::readWord(WORD address, WORD &dest) {
	if (address < 0x4000) {
		dest = (rom_bank_0_[address] << 8) | (rom_bank_0_[address+1]);
	} else if (address < 0x8000) {
		dest = (switchable_rom_bank_[curr_rom_bank_][address&0x3FFF] << 8) | 
			(switchable_rom_bank_[curr_rom_bank_][address&0x3FFF+1]);
	} else if (address < 0xA000) {
		dest = (video_ram_[address&0x1FFF] << 8) | video_ram_[address&0x1FFF+1];
	} else if (address < 0xC000) {
		dest = (switchable_ram_bank_[curr_ram_bank_][address&0x1FFF] << 8) |
			switchable_ram_bank_[curr_ram_bank_][address&0x1FFF+1];
	} else if (address < 0xE000) {
		dest = (internal_ram_[address&0x1FFF] << 8) | internal_ram_[address&0x1FFF+1];
	} else if (address >= 0xFE00 && address < 0xFEA0) {
		dest = (oam_[address&0x9F] << 8) | oam_[address&0x9F+1];
	} else if (address >= 0xFF00 && address < 0xFF4C) {
		dest = (io_ports_[address&0x4B] << 8) | io_ports_[address&0x4B+1];
	} else if (address >= 0xFF80 && address < 0xFFFF) {
		dest = (stack_ram_[address&0x7E] << 8) | stack_ram_[address&0x7E+1];
	} else if (address == 0xFFFF) {
...
void MMU::writeWord(WORD address, WORD val) {
	if (address < 0x4000) {
		...
		if ((io_ports_[0x41] & 0x03) != 0x03) {
			video_ram_[address&0x1FFF] = (val >> 8) & 0x00FF;
			video_ram_[address&0x1FFF+1] = (val & 0x00FF);
		}
	} else if (address < 0xC000) {
		if (ram_enable_) {
			switchable_ram_bank_[curr_ram_bank_][address&0x1FFF] = (val >> 8) & 0x00FF;
			switchable_ram_bank_[curr_ram_bank_][address&0x1FFF+1] = (val & 0x00FF);
		}
	} else if (address < 0xE000) {
		internal_ram_[address&0x1FFF] = (val >> 8) & 0x00FF;
		internal_ram_[address&0x1FFF+1] = (val & 0x00FF);
	} else if (address >= 0xFE00 && address < 0xFEA0) {
		// if STAT register (0xFF41) shows we are in H-Blank or V-Blank
		// we can write to OAM sprite memory
		if (!(io_ports_[0x41] & 0x02)) {
			oam_[address&0x9F] = (val >> 8) & 0x00FF;
			oam_[address&0x9F+1] = (val & 0x00FF);
		}
	} else if (address >= 0xFF00 && address < 0xFF4C) {
		std::cout << "Writing to I/O registers with WORD, consider changing to BYTE since this fails.\n";
	} else if (address >= 0xFF80 && address < 0xFFFF) {
		stack_ram_[address&0x7E] = (val >> 8) & 0x00FF;
		stack_ram_[address&0x7E+1] = (val & 0x00FF);
	} else if (address == 0xFFFF) {
